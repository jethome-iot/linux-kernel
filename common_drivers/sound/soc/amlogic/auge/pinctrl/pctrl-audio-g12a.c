...
struct pinctrl_pin_desc g12a_audio_pins[] = {
	AUDIO_PIN(TDM_D0),
	AUDIO_PIN(TDM_D1),
	AUDIO_PIN(TDM_D2),
	AUDIO_PIN(TDM_D3),
...
/* tdm oe pin */
static const unsigned int tdma_oe_pins[] = {TDMA_OE};
static const unsigned int tdmb_oe_pins[] = {TDMB_OE};
static const unsigned int tdmc_oe_pins[] = {TDMC_OE};
...
static const char * const tdm_groups[] = {
	...
	"tdm_d30", "tdm_d31", "tdma_oe", "tdmb_oe", "tdmc_oe"
};
		...
		unsigned int selector, unsigned int group)
{
	...
	if (selector <= FUNC_TDM_DIN_LAST) {
		...
		if (group == 47) {
			...
			if (selector / 8 == 0) {
				val = selector % 8;
				aml_audiobus_update_bits(actrl, EE_AUDIO_TDMOUT_A_CTRL0,
					0x1 << (24 + val), 0 << (24 + val));
				aml_audiobus_update_bits(actrl, EE_AUDIO_TDMOUT_A_CTRL1,
					0x1 << (0 + val), 0 << (0 + val));			}
		} else if (group == 48) {
			/* tdmina */
			if (selector / 8 == 1) {
				val = selector % 8;
				aml_audiobus_update_bits(actrl, EE_AUDIO_TDMOUT_B_CTRL0,
					0x1 << (24 + val), 0 << (24 + val));
				aml_audiobus_update_bits(actrl, EE_AUDIO_TDMOUT_B_CTRL1,
					0x1 << (0 + val), 0 << (0 + val));			}
		} else if (group == 49) {
			/* tdmina */
			if (selector / 8 == 2) {
				val = selector % 8;
				aml_audiobus_update_bits(actrl, EE_AUDIO_TDMOUT_C_CTRL0,
				0x1 << (24 + val), 0 << (24 + val));
				aml_audiobus_update_bits(actrl, EE_AUDIO_TDMOUT_C_CTRL1,
				0x1 << (0 + val), 0 << (0 + val));
			}
		}

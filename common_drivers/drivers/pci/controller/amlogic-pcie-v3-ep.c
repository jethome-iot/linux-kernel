...
static int ep_link_times = WAIT_LINKUP_TIMEOUT - 10;
module_param(ep_link_times, int, 0644);
MODULE_PARM_DESC(ep_link_times, "select pcie ep link speed ");
...
static int amlogic_pcie_ep_write_header(struct pci_epc *epc, u8 func_no, u8 vfunc_no,
...
static int amlogic_pcie_ep_set_bar(struct pci_epc *epc, u8 func_no, u8 vfunc_no,
				   struct pci_epf_bar *epf_bar)
{
	...
	if (((flags & PCI_BASE_ADDRESS_SPACE) == PCI_BASE_ADDRESS_SPACE_IO)) {
		...
		if (is_64bits && is_prefetch) {
			reg |= PCIE_BAR_MEM_TYPE_64 |
				...
				PCIE_BAR_MEM_MASK;
			amlogic_pcie_ep_fn_writel(amlogic, func_no,
						  PCIE_BAR_0 + bar * 4,
						  lower_32_bits(reg));
			bar += 1;
			amlogic_pcie_ep_fn_writel(amlogic, func_no,
	...
	amlogic_pcie_ep_fn_writel(amlogic, func_no, PCIE_BAR_0 + bar * 4, reg);
...
static void amlogic_pcie_ep_clear_bar(struct pci_epc *epc, u8 func_no, u8 vfunc_no,
				       struct pci_epf_bar *epf_bar)
{
	struct amlogic_pcie_ep *ep = epc_get_drvdata(epc);
	struct amlogic_pcie *amlogic = &ep->amlogic;
...
static int amlogic_pcie_ep_map_addr(struct pci_epc *epc, u8 func_no, u8 vfunc_no,
...
static void amlogic_pcie_ep_unmap_addr(struct pci_epc *epc, u8 func_no, u8 vfunc_no,
					phys_addr_t addr)
{
	struct amlogic_pcie_ep *ep = epc_get_drvdata(epc);
	struct amlogic_pcie *amlogic = &ep->amlogic;
...
static int amlogic_pcie_ep_set_msi(struct pci_epc *epc, u8 func_no, u8 vfunc_no,
				   u8 mmc)
{
	struct amlogic_pcie_ep *ep = epc_get_drvdata(epc);
	struct amlogic_pcie *amlogic = &ep->amlogic;
	 ...
	 */
	flags = amlogic_pcie_ep_fn_readw(amlogic, func_no,
					 EP_FUNC_MSI_CAP_OFFSET +
					 PCI_MSI_FLAGS);
	flags = (flags & ~PCI_MSI_FLAGS_QMASK) | (mmc << 1);
	flags |= PCI_MSI_FLAGS_64BIT;
	flags &= ~PCI_MSI_FLAGS_MASKBIT;
	amlogic_pcie_ep_fn_writew(amlogic, func_no,
				  EP_FUNC_MSI_CAP_OFFSET +
				  PCI_MSI_FLAGS, flags);
...
static int amlogic_pcie_ep_get_msi(struct pci_epc *epc, u8 func_no, u8 vfunc_no)
{
	struct amlogic_pcie_ep *ep = epc_get_drvdata(epc);
	struct amlogic_pcie *amlogic = &ep->amlogic;
	u16 flags, mme;
...
static int amlogic_pcie_ep_send_legacy_irq(struct amlogic_pcie_ep *ep, u8 func_no,
					   u8 intx)
{
	struct amlogic_pcie *amlogic = &ep->amlogic;
	u16 cmd;
	...
	cmd = amlogic_pcie_ep_fn_readw(&ep->amlogic, func_no,
				       PCI_COMMAND);
...
static int amlogic_pcie_ep_send_msi_irq(struct amlogic_pcie_ep *ep, u8 func_no,
					 u8 interrupt_num)
{
	struct amlogic_pcie *amlogic = &ep->amlogic;
	u32 val = 0;
...
static int amlogic_pcie_ep_raise_irq(struct pci_epc *epc, u8 func_no, u8 vfunc_no,
				     ...
				     u16 interrupt_num)
{
	...
	case PCI_EPC_IRQ_LEGACY:
		return amlogic_pcie_ep_send_legacy_irq(ep, func_no, 0);
	case PCI_EPC_IRQ_MSI:
		return amlogic_pcie_ep_send_msi_irq(ep, func_no, interrupt_num);
	default:
		return -EINVAL;
	}
}

static int amlogic_pcie_ep_init_port_for_m31_phy(struct amlogic_pcie *amlogic)
{
	u32 val;
	...
	val = readl(amlogic->phy_base + 0x470);
	val |= (1 << 6);
	writel(val, amlogic->phy_base + 0x470);
...
static int amlogic_pcie_ep_init_port_for_aml_phy(struct amlogic_pcie *amlogic)
{
	...
	/* set phy pll */
	if (of_property_read_bool(dev_of_node(dev), "phy_pll_setting"))
		ret = amlogic_pcie_set_phy_pll(amlogic);

	/* debug GEN3 form zhang yong*/
	for (i = 0; i < 4; i++) {
...
static int amlogic_pcie_ep_init_port(struct amlogic_pcie *amlogic)
{
	...
	case AMLOGIC_PHY:
		dev_dbg(dev, " pcie init ep port and AMLOGIC_PHY\n");
		ret = amlogic_pcie_ep_init_port_for_aml_phy(amlogic);
		break;
	case M31_COMBPHY:
		dev_dbg(dev, " pcie init ep port and M31_COMBPHY\n");
		break;
	case M31_PHY:
	default:
		dev_dbg(dev, " pcie init ep port and M31_PHY\n");
		ret = amlogic_pcie_ep_init_port_for_m31_phy(amlogic);
		break;
	}
	...
	// Loopback Master
	//regs = amlogic_pciectrl_read(amlogic, PCIE_A_CTRL3);
	//regs |= (1<<2);
	//amlogic_pciectrl_write(amlogic, regs, PCIE_A_CTRL3);

	return ret;
}

static void amlogic_pcie_ep_start_link(struct amlogic_pcie *amlogic)
{
	struct device *dev = amlogic->dev;
	u32 regs;
...
static void amlogic_reset_gpio_irq_work(struct work_struct *work)
{
	...
	usleep_range(20, 100);
	amlogic_pcie_ep_start_link(amlogic);
}
...
amlogic_pcie_ep_get_features(struct pci_epc *epc, u8 func_no, u8 vfunc_no)
{
...
static int amlogic_pcie_ep_probe(struct platform_device *pdev)
{
	...
	if (ep_link_times)
		amlogic->link_times = ep_link_times;
	...
	//TODO set phy pll
	//err = amlogic_pcie_ep_init_port(amlogic);
	//if (err)
	//	return err;

	err = pci_epc_mem_init(epc, ep->mem_res->start,
			       resource_size(ep->mem_res), PAGE_SIZE);
	if (err < 0) {
...
static int amlogic_pcie_ep_remove(struct platform_device *pdev)
{
	struct amlogic_pcie_ep *ep = platform_get_drvdata(pdev);
	struct amlogic_pcie *amlogic = &ep->amlogic;
...
static struct platform_driver amlogic_pcie_ep_driver = {
	...
	.remove = amlogic_pcie_ep_remove,
};

int __init aml_pcie_ep_v3_init(void)
{
	return platform_driver_register(&amlogic_pcie_ep_driver);
}

void __exit aml_pcie_ep_v3_exit(void)
{
	platform_driver_unregister(&amlogic_pcie_ep_driver);
}

/*
 * module_platform_driver(amlogic_pcie_ep_driver);
 *
 * MODULE_AUTHOR("Amlogic Inc");
 * MODULE_DESCRIPTION("Amlogic AXI PCIe EP driver");

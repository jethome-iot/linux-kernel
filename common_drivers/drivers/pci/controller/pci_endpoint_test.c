...
#ifndef CONFIG_AMLOGIC_PCIE_V3_EP
		ret = irq_type;
#else
		ret = test->irq_type;
#endif
...
int __init aml_pcie_ep_test_init(void)
{
	return pci_register_driver(&pci_endpoint_test_driver);
}

void __exit aml_pcie_ep_test_exit(void)
{
	pci_unregister_driver(&pci_endpoint_test_driver);
}

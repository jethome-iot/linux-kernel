...
static void txhd2_dmc_port_config(struct ddr_bandwidth *db, int channel, int port)
{
	unsigned int val;
	unsigned int off = 0;
	int subport = -1;
...
static void txhd2_dmc_range_config(struct ddr_bandwidth *db, int channel,
...
static void txhd2_dmc_bandwidth_enable(struct ddr_bandwidth *db)
{
	unsigned int val;
...
static void txhd2_dmc_bandwidth_init(struct ddr_bandwidth *db)
{
	unsigned int i;
	...
	for (i = 0; i < db->channels; i++) {
		if (!db->port[i])
			txhd2_dmc_port_config(db, i, -1);
		txhd2_dmc_range_config(db, i, 0, 0xffffffff);
		db->range[i].start = 0;
		db->range[i].end   = 0xffffffff;
	}
}

static int txhd2_handle_irq(struct ddr_bandwidth *db, struct ddr_grant *dg)
{
...
static int txhd2_dump_reg(struct ddr_bandwidth *db, char *buf)
{
	int s = 0, i;
	unsigned int r, off;
...
struct ddr_bandwidth_ops txhd2_ddr_bw_ops = {
	.init             = txhd2_dmc_bandwidth_init,
	.config_port      = txhd2_dmc_port_config,
	.config_range     = txhd2_dmc_range_config,
	.get_freq         = txhd2_get_dmc_freq_quick,
	.handle_irq       = txhd2_handle_irq,
	.bandwidth_enable = txhd2_dmc_bandwidth_enable,
#if DDR_BANDWIDTH_DEBUG
	.dump_reg         = txhd2_dump_reg,
#endif
};

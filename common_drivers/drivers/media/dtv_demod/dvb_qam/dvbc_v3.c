...
#define MSIZE 8
	...
	if (cpu_after_eq(MESON_CPU_MAJOR_ID_T5M)) {
		if (dis_idx[0] < 2 && dis_idx[1] < 2) {
			...
			for (i = 0; i < greater; i++) {
				temp = matrix[row[i]][col[i]];
				if (row[i] > 0 && row[i] < 7 && col[i] > 0 && col[i] < 7) {
					if (temp > matrix[row[i] - 1][col[i]] &&
						temp > matrix[row[i] + 1][col[i]] &&
						temp > matrix[row[i]][col[i] - 1] &&
						temp > matrix[row[i]][col[i] + 1]) {
						++count;
					}
				} else if (row[i] == 0 && col[i] > 0 && col[i] < 7) {
					if (temp > matrix[row[i] + 1][col[i]] &&
						temp > matrix[row[i]][col[i] - 1] &&
						temp > matrix[row[i]][col[i] + 1]) {
						++count;
					}
				} else if (col[i] == 0 && row[i] > 0 && row[i] < 7) {
					if (temp > matrix[row[i] - 1][col[i]] &&
						temp > matrix[row[i] + 1][col[i]] &&
						temp > matrix[row[i]][col[i] + 1]) {
						++count;
					}
				} else if (row[i] == 7 && col[i] > 0 && col[i] < 7) {
					if (temp > matrix[row[i] - 1][col[i]] &&
						temp > matrix[row[i]][col[i] - 1] &&
						temp > matrix[row[i]][col[i] + 1]) {
						++count;
					}
				} else if (col[i] == 7 && row[i] > 0 && row[i] < 7) {
					if (temp > matrix[row[i] - 1][col[i]] &&
						temp > matrix[row[i]][col[i] - 1] &&
						temp > matrix[row[i] + 1][col[i]]) {
						++count;
				...
				if (col[i] == 0 || col[i] == 2 || col[i] == 3 ||
					col[i] == 5 || col[i] == 6)
					flag = 1;
			}

			PR_DVBC("greater: %d, count: %d, flag: %d\n",
					greater, count, flag);
			...
			if (count == 8 && !flag) {
				...
				PR_DVBC("in count == 8 and flag == 0  32\n");
			}
		}
	}

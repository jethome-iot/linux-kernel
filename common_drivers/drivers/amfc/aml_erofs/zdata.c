...
#ifdef CONFIG_AMLOGIC_EROFS_PCPU_KTHREAD
...
static struct kthread_worker __nocfi *erofs_init_percpu_worker(int cpu)
{
	struct kthread_worker *worker =
		f_kthread_create_worker_on_cpu(cpu, 0, "erofs_worker/%u", cpu);

	if (IS_ERR(worker))
		return worker;
	if (IS_ENABLED(CONFIG_AMLOGIC_EROFS_PCPU_KTHREAD_HIPRI))
...
#if defined(CONFIG_HOTPLUG_CPU) && defined(CONFIG_AMLOGIC_EROFS_PCPU_KTHREAD)
...
#else /* !CONFIG_HOTPLUG_CPU || !CONFIG_AMLOGIC_EROFS_PCPU_KTHREAD */
...
static struct page __nocfi *pickup_page_for_submission(struct z_erofs_pcluster *pcl,
					       unsigned int nr,
					       struct page **pagepool,
					       struct address_space *mc)
{
...
out_tocache:
	if (!tocache || f_add_to_page_cache_lru(page, mc, index + nr, gfp)) {
 ...
 */
static void __nocfi z_erofs_pcluster_readmore(struct z_erofs_decompress_frontend *f,
				      ...
				      bool backmost)
{
	...
	if (backmost) {
		...
		if (rac) {
			...
			f_readahead_expand(rac, newstart, cur - newstart);
			return;
		}
...
static int z_erofs_readpage(struct file *file, struct page *page)
{
	...
	trace_aerofs_readpage(page, false);
...
static void z_erofs_readahead(struct readahead_control *rac)
{
	...
	trace_aerofs_readpages(inode, readahead_index(rac), nr_pages, false);

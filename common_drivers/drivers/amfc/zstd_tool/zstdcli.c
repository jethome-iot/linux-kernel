                         ...
                         int compressionLevel, ZSTD_compressionParameters comprParams)
{
	FILE *in;
	FILE *out;
	unsigned int file_size = 0, count;
	unsigned char *in_buff = NULL, *out_buff = NULL;
	...
	file_size = UTIL_getFileSize(srcFileName);
	in = fopen(srcFileName, "r");
	if (!dstFileName) {
		sprintf(buf, "%s.zstd", srcFileName);
		out = fopen(buf, "w+");
	} else {
		out = fopen(dstFileName, "w+");
	}
	if (!in || !out) {
		printf("open file failed:%p %p, dstFileName:%s\n", in, out, dstFileName);
		return -1;
	}
	in_buff = malloc(1024 * 1024 * 4);
	out_buff = malloc(1024 * 1024 * 5);
	if (!in_buff || !out_buff) {
		printf("malloc buff failed\n");
		return -1;
	}
	params = ZSTD_getParams(compressionLevel, 1024 * 1024 * 4, 0);
	wksp_size = ZSTD_estimateCCtxSize_usingCParams(params.cParams);
	cwksp = malloc(wksp_size);
	if (!cwksp)
		return -1;
	...
	while (file_size) {
		if (file_size >= 4096 * 1024)
			...
			count = file_size;
		fread(in_buff, count, 1, in);
		zstd_cctx_init(cctx, &params, count);
		ret = ZSTD_compress2(cctx, out_buff, count, in_buff, count);
		if (ZSTD_isError(ret)) {
...
int main(int argCount, const char* argv[])
{
    ...
    if (operation==zom_compress) {
        ...
        if ((filenames->tableSize==1) && outFileName) {
            ...
            operationResult = zstd_fragment_file_compress(fCtx, prefs, outFileName, filenames->fileNames[0], dictFileName, cLevel, compressionParams);
        } else {

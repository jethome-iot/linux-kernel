...
static int spi_nor_spimem_exec_op(struct spi_nor *nor, struct spi_mem_op *op)
{
	...
	return meson_spi_mem_exec_op(nor->spimem, op);
}
					...
					size_t len, u8 *buf)
{
	...
	if (nor->dirmap.rdesc) {
		nbytes = meson_spi_mem_dirmap_read(nor->dirmap.rdesc, op.addr.val,
					 ...
					 size_t len, const u8 *buf)
{
	...
	if (nor->dirmap.wdesc) {
		nbytes = meson_spi_mem_dirmap_write(nor->dirmap.wdesc, op.addr.val,
...
static int spi_nor_select_erase(struct spi_nor *nor)
{
	 ...
	 * size, when possible.
	 */
#ifdef CONFIG_AMLOGIC_MTD_SPI_NOR_USE_4K_SECTORS
